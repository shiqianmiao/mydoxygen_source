                                  ...
                                  )
{
  ...
  else // not an enum value or anonymous compound
  {
    ...
    {
      //自定义，修改包接口详情页面，接口标题由SaleServiceApp::newSale 改为Sale.newSale
      QCString memberCustom = substitute(ldef, "::",sep);
      int pos = memberCustom.find("ServiceApp");
      int pos2 = memberCustom.find("::");
      int lenPos = pos2 - pos;
      if (pos > 0 && lenPos > 0) {
          memberCustom = memberCustom.replace(pos, lenPos, "");
          memberCustom = memberCustom.replace(pos, 2, ".");
      }
      //自定义内容结束
