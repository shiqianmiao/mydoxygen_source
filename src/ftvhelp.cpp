...
void FTVHelp::generateLink(FTextStream &t,FTVNode *n)
{
  ...
  else // link into other frame
  {
    ...
    if (m_topLevelIndex)
      ...
      t << "\" target=\"_self\">";
    //自定义, 修改包列表里面的名称，过滤掉ServiceApp
    QCString cname = n->name;
    int pos = (int) cname.find("ServiceApp");
    int line = cname.find("_");
    int pos2 = 0;
    if (line > 0) {
        int verLen = cname.length() - line;
        pos2 = cname.length() - 10 - verLen;
    } else {
        pos2 = cname.length() - 10;
    }
    cname = cname.replace(pos2, 10, "");
    t << convertToHtml(cname);
    //自定义结束
...
void FTVHelp::generateTree(FTextStream &t, const QList<FTVNode> &nl,int level,int maxLevel,int &index)
{
  ...
  for (nli.toFirst();(n=nli.current());++nli)
  {
    ...
    else // leaf node
    {
      ...
      {
        //自定义，修改类文件列表里面的C 改为包
        t << "<span class=\"icona\"><span class=\"icon\">包</span></span>";
      }
      else
      {
      ...
      {
        //自定义包列表的包描述的颜色
        t << "<font style='color:#0099cc;'>包简介：</font>";
        t << "<font style='color:#99cc00;'>";
        generateBriefDoc(t,n->def);
        t << "</font>";
      }
      t << "</td></tr>" << endl;
